...
def test_safe_test_id_basic():
    assert _safe_test_id("test/foo.py::test_bar") == "test_foo.py__test_bar"


def test_safe_test_id_replaces_unsafe_chars():
    sid = _safe_test_id("test/foo.py::test_bar[a/b c]")
    assert "/" not in sid
    assert " " not in sid


def test_safe_test_id_caps_length():
    long = "x" * 500
    sid = _safe_test_id(long)
    assert len(sid) <= 200


def test_initialize_refuses_non_empty_dir(tmp_path):
...
def test_initialize_creates_cases_subdir(tmp_path):
    root = tmp_path / "out"
    od = OutputDir(root)
    od.initialize()
...
def test_run_artifact_written_to_root(tmp_path):
    root = tmp_path / "out"
    od = OutputDir(root)
    od.initialize()
...
def test_case_artifact_written_under_per_kind_subdir(tmp_path):
    root = tmp_path / "out"
    od = OutputDir(root)
    od.initialize()
...
def test_two_kinds_under_same_case(tmp_path):
    root = tmp_path / "out"
    od = OutputDir(root)
    od.initialize()
...
def test_record_case_artifact_does_not_double_write(tmp_path):
...
def test_finalize_writes_manifest_v2(tmp_path):
    root = tmp_path / "out"
    od = OutputDir(root)
    od.initialize()
...
def test_finalize_creates_run_meta_json(tmp_path):
    root = tmp_path / "out"
    od = OutputDir(root)
    od.initialize()
