...
from pytest_prism.api import RenderResult
...
def test_loads_session_hooks_from_entry_points():
...
def test_kind_collision_is_hard_error():
    eps = [
...
def test_import_failure_skipped_by_default(caplog):
...
def test_import_failure_raises_in_strict_mode():
    bad = EntryPoint(name="broken", value="nonexistent_pkg:Thing", group="pytest_prism.renderers")
    with (
