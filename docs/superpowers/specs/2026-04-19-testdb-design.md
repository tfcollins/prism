...
- JWT in `httpOnly` cookie. A bootstrap user is created from `TESTDB_ADMIN_EMAIL` / `TESTDB_ADMIN_PASSWORD` env vars on the api container's first start; once at least one user exists, those env vars are ignored. Any authenticated user may create or delete other users — the v1 trust model is flat.
